  ...
  controller.listaPorConta = function(req, res){
	...
	controller.obtemLaudo = function(req, res){
		...
		Laudo.findById(id,{include:[{
			...
			model:models.Intervencao, include:[{
				model:models.Perigo, include:[{model:models.Risco}]
			},{
				model:models.Risco
			},{
				model:models.SistemaSeguranca2
			},{
				model:models.SistemaSeguranca
			},{
				model:models.Miparl
			},{
				model:models.Miparl61
			}]
		},{
			...
			model:models.DisposicaoFinal
		},{
			model:models.InstalacoesDispEletrico
		},{
	...
	controller.removeLaudo = function(req, res){
		Laudo.findById(req.params.id, {include:[{model:models.Dispositivo}]}).then(function(data){
			...
			if(data.status === "Aprovado") {
				...
				if(data.intervencoes != undefined){
					data.intervencoes.forEach(function(interv){
						controller.deleteFolderRecursive('./files/dispositivos/'+interv.id);
					});
				}
    ...
    var fotopp01, fotopp02, fotopp03, fotoit01;
		var imagem;
		var sub_path;
		var path;
		var path_novo = './files/';
	...
	console.log("Date: ", new Date());
		...
		req.body.dispositivos.forEach(function(disp){
			...
			req.body.intervencoes.forEach(function(it){

				if(it.fotoit01 != undefined){
					it.pathAntigo1 = it.fotoit01.path;
					it.fotoit01 = it.fotoit01.nome;
				}
			...
			models.Maquina.all({include:[models.Cliente]}).then(function(machineData){
				...
				Laudo.all({include: [{model:models.Maquina, include:[{model:models.Cliente}]}]}).then(
					...
					function(data){
					Laudo.create(req.body, {include:[{model:models.Dispositivo},{model:models.Intervencao},{model:models.PontoPerigo}]}).then(function(laudo){

						if(fotopp01 != undefined){
						 	sub_path = '/fotopp01/';
							laudoId = laudo.id;

						 	controller.salvaImagemPontoPerigo(path_novo, req.body.pathAntigo,
						...
						if(fotoit01 != undefined){
							sub_path = '/fotoit01/';
							laudoId = data.id;
							controller.salvaImagemIntervencao(path_novo, req.body.pathAntigo,
								   req.body.intervencoes.fotoit01, sub_path, laudo.id, false);
						   }
					...
					laudo.get().dispositivos.forEach(function(dispositivo){
						...
						laudo.get().intervencoes.forEach(function(intervencao){
							req.body.intervencoes.forEach(function(interv){
								if(intervencao.get().intervencao == interv.intervencao){
									// salva as imagens de intervencoes
									if (interv.fotoit01){
										sub_path = '/fotoit01/';
										controller.salvaImagemIntervencao(path_novo, interv.pathAntigo1,
											 interv.fotoit01, intervencao.id, sub_path, laudo.id, false);
									}
								
									// Cadastra o perigos
									if(interv.Perigos){
										interv.Perigos.forEach(function(perigo){
											intervencao.addPerigo(perigo.id);
										});
									}
									// Cadastra os riscos
									if(interv.Riscos){
										interv.Riscos.forEach(function(risco){
											intervencao.addRisco(risco.id);
										});
									}
									// Cadastra os miparls
									if(interv.Miparls){
										interv.Miparls.forEach(function(miparl){
											intervencao.addMiparls(miparl.id);
										});
									}

									if(interv.miparls61){
										interv.miparls61.forEach(function(miparl61){
											intervencao.addMiparls61(miparl61.id);
										});
									}
									// Cadastra o sistema de segurança
									
									if(interv.SistemaSegurancas){
										interv.SistemaSegurancas.forEach(function(sistemaSeg){
											intervencao.addSistemaSegurancas(sistemaSeg.id);
										});
									}
									if(interv.SistemaSeguranca2s){
										interv.SistemaSeguranca2s.forEach(function(sistemaSeg2){
											intervencao.addSistemaSeguranca2s(sistemaSeg2.id);
										});
									}
								}
							});
						...
						if(req.body.manuais){
							...
							req.body.manuais.forEach(function(manual, index){
								laudo.addManual(manual.id);
							});
						}
						
						if(req.body.instalacoesDispEletricos){
							req.body.instalacoesDispEletricos.forEach(function(instalacoesDispEletrico, index){
								laudo.addInstalacoesDispEletrico(instalacoesDispEletrico.id);
							});
						}
						...
						if(req.body.disposicaoFinais){
							req.body.disposicaoFinais.forEach(function(disposicaoFinal, index){
								laudo.addDisposicaoFinal(disposicaoFinal.id);
							});
						}
			...
			return Laudo.findById(req.params.id/*,{include:[{all:true, nested:true}]}*/).then(function(laudo){
				...
				if(req.body.destroyDispositivos){
				 ...
				 if(req.body.destroyPontoPerigo){
				  ...
				  if(req.body.destroyIntervencao){
				 	req.body.destroyIntervencao.forEach(function(interv){

				 		if(interv.fotoit01 != undefined){
				 		 	 controller.deleteFolderRecursive('./files/laudos/'+laudo.id+'/intervencao/'+interv.id);

				 		 }

				 		
				 		 models.Intervencao.destroy({where:{id:interv.id}});
				 	});
				 }
...
/////////////////////////////////////////////////////////////////////////////////
				...
				if(req.body.intervencoes){
					
					req.body.intervencoes.forEach(function(interv){
						
					if(interv.fotoit01){

						interv.pathAntigo1 = interv.fotoit01.path;
						interv.fotoit01 = interv.fotoit01.nome;
					}
					
					// atualiza intervenções
					if(interv.id){

						models.Intervencao.update(interv, {where:{id:interv.id}}).then(function(){

								if(interv.fotoit01 != undefined){
									sub_path = "/fotoit01/"
								controller.salvaImagemIntervencao(path_novo, interv.pathAntigo1, interv.fotoit01, interv.id, sub_path, laudo.id, true);
								}

	
						models.Intervencao.findById(interv.id).then(function(newIntervencao){

							newIntervencao.setPerigos([]).then(function(){
									if(interv.Perigos){
										interv.Perigos.forEach(function(perigo){
											newIntervencao.addPerigo(perigo.id);
										});
									}
							});

							newIntervencao.setRiscos([]).then(function(){
								if(interv.Riscos){
									interv.Riscos.forEach(function(risco){
										newIntervencao.addRiscos(risco.id);
									});
								}
							});

							newIntervencao.setMiparls([]).then(function(){
								if(interv.Miparls){
									interv.Miparls.forEach(function(miparl){
										newIntervencao.addMiparls(miparl.id);
									});
								}
							});

							newIntervencao.setMiparls61([]).then(function(){
								if(interv.miparls61){
									interv.miparls61.forEach(function(miparl61){
										newIntervencao.addMiparl61(miparl61.id);
									});
								}
							});
							
							newIntervencao.setSistemaSegurancas([]).then(function(){
								if(interv.SistemaSegurancas){
									interv.SistemaSegurancas.forEach(function(sistemaSeg){
										newIntervencao.addSistemaSegurancas(sistemaSeg.id);
									});
								}
							});
							newIntervencao.setSistemaSeguranca2s([]).then(function(){
								if(interv.SistemaSeguranca2s){
									interv.SistemaSeguranca2s.forEach(function(sistemaSeg2){
										newIntervencao.addSistemaSeguranca2s(sistemaSeg2.id);
									});
								}
							});
					...
					}else{  // cria nova intervenção
						var intervencao = models.Intervencao.build(interv);

						intervencao.laudoId = laudo.id;

						intervencao.save().then(function(newIntervencao){

							if(interv.fotoit01 != undefined){
								sub_path = "/fotoit01/"
								controller.salvaImagemIntervencao(path_novo, interv.pathAntigo1, interv.fotoit01, newIntervencao.id, sub_path, laudo.id, false);
							}
	
							interv.Perigos.forEach(function(perigo){
								newIntervencao.addPerigo(perigo.id)
									perigo.Riscos.forEach(function(risco){
									newIntervencao.addRisco(risco.id);
									});
							});
				...
				if(req.body.manuais){
					...
					models.Manual.findAll({
						where:{
							id:{ $in : req.body.manuais.map(function(item){
								return item.id;
							})}
						}
					}).then(function(manuais){
					
						laudo.setManuais(manuais);
					});
				}

				if(req.body.disposicaoFinais){
				
					models.DisposicaoFinal.findAll({
						where:{
							id:{ $in : req.body.disposicaoFinais.map(function(item){
								return item.id;
							})}
						}
					}).then(function(disposicaoFinais){
					
						laudo.setDisposicaoFinais(disposicaoFinais);
					});
				}
				
				if(req.body.instalacoesDispEletricos){
				
					models.InstalacoesDispEletrico.findAll({
						where:{
							id:{ $in : req.body.instalacoesDispEletricos.map(function(item){
								return item.id;
							})}
						}
					}).then(function(instalacoesDispEletricos){
					
						laudo.setInstalacoesDispEletricos(instalacoesDispEletricos);
					});
				}
	...
	controller.salvaImagemIntervencao = function(path_novo, path_antigo, nome, idIntervencao, sub_path, laudoId, deletar){

		var path_novo_completo = path_novo + 'laudos/'+laudoId+'/intervencao/'+idIntervencao+sub_path;
